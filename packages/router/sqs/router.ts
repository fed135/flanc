...
import { deferred } from '@<project_name>/core-util/async';
import { InternalError } from '@<project_name>/core-util/errors';
import Logger from '@<project_name>/core-util/logging';
        ...
        (messageBody.Records || [messageBody]).forEach((body) => {
          pollers[queue].events.forEach((event: Route) => {
            ...
            if (messageOperationId === event.operationId) {
              ...
              startWebTransaction(`SQS ${messageOperationId}`, () => {
                Promise.resolve(event.resolver(context))
